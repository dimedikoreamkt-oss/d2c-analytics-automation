...
with_ma AS (
  SELECT
    *,
    -- 7일 이동평균 (window framing OK for AVG)
    AVG(impressions) OVER (
      PARTITION BY keyword, campaign_name ORDER BY event_date
    ...
    -- LAG는 framing 없이
    LAG(impressions, 7) OVER (PARTITION BY keyword, campaign_name ORDER BY event_date) AS prev_week_impressions,
    LAG(clicks, 7)      OVER (PARTITION BY keyword, campaign_name ORDER BY event_date) AS prev_week_clicks,
    LAG(cost_krw, 7)    OVER (PARTITION BY keyword, campaign_name ORDER BY event_date) AS prev_week_cost,
    LAG(revenue, 7)     OVER (PARTITION BY keyword, campaign_name ORDER BY event_date) AS prev_week_revenue
  FROM kw_daily
),
with_trend AS (
  SELECT
    *,
    LAG(rev_7d_avg, 7) OVER (PARTITION BY keyword, campaign_name ORDER BY event_date) AS prev_rev_7d_avg
  FROM with_ma
)
SELECT
  event_date, keyword, campaign_name, campaign_type,
  ...
  CASE
    WHEN SAFE_DIVIDE(rev_7d_avg - prev_rev_7d_avg, NULLIF(prev_rev_7d_avg, 0)) >= 0.15 THEN '📈 상승'
    WHEN SAFE_DIVIDE(rev_7d_avg - prev_rev_7d_avg, NULLIF(prev_rev_7d_avg, 0)) <= -0.15 THEN '📉 하락'
    ELSE '➡️ 횡보'
