...
// ===== 광고 지표 가중평균 계산 (CPC/CPM/CTR/CAC/CPA/ROAS/AOV 등) =====
// 광고 산업 표준: 개별 값을 SUM하면 안 되고, 원시 데이터로 재계산해야 함
function weightedMetric(rows, k) {
  const s = (key) => rows.reduce((a, r) => {
    const v = parseFloat(r[key]);
    return isFinite(v) ? a + v : a;
  }, 0);

  switch(k) {
    // 광고 효율 지표
    case 'cpc':               return s('clicks') > 0 ? s('spend_krw') / s('clicks') : 0;
    case 'cpm':               return s('impressions') > 0 ? (s('spend_krw') / s('impressions')) * 1000 : 0;
    case 'ctr_pct':           return s('impressions') > 0 ? (s('clicks') / s('impressions')) * 100 : 0;
    case 'click_to_purchase_pct': return s('clicks') > 0 ? (s('purchases') / s('clicks')) * 100 : 0;

    // 획득/전환 비용
    case 'cac_krw':           return s('acquired_users') > 0 ? s('spend_krw') / s('acquired_users') : 0;
    case 'cpa_krw':           return s('purchases') > 0 ? s('spend_krw') / s('purchases') : 0;

    // 매출 배수
    case 'roas':              return s('spend_krw') > 0 ? s('ga_revenue') / s('spend_krw') : 0;
    case 'platform_roas':     return s('spend_krw') > 0 ? s('platform_revenue') / s('spend_krw') : 0;
    case 'ltv_cac_ratio':     return s('spend_krw') > 0 && s('acquired_users') > 0 ?
                                     (s('ltv_estimate_krw') / s('acquired_users')) / (s('spend_krw') / s('acquired_users')) : 0;

    // 단가/전환율
    case 'aov':               return s('purchases') > 0 ? s('revenue') / s('purchases') : 0;
    case 'session_cvr_pct':   return s('sessions') > 0 ? (s('purchases') / s('sessions')) * 100 : 0;
    case 'user_cvr_pct':      return s('users') > 0 ? (s('purchases') / s('users')) * 100 : 0;
    case 'atc_rate_pct':      return s('sessions') > 0 ? (s('add_to_carts') / s('sessions')) * 100 : 0;
    case 'cart_abandonment_pct': return s('add_to_carts') > 0 ?
                                     ((s('add_to_carts') - s('purchases')) / s('add_to_carts')) * 100 : 0;

    // 유저당 매출
    case 'revenue_per_user':           return s('users') > 0 ? s('revenue') / s('users') : 0;
    case 'revenue_per_acquired_user':  return s('acquired_users') > 0 ? s('revenue') / s('acquired_users') : 0;
    case 'ltv_estimate_krw':           return s('acquired_users') > 0 ? s('ltv_estimate_krw') / s('acquired_users') : NaN;

    default: return NaN;
  }
}

// 계산형 지표(가중평균 적용 대상)인지 판정
function isCalculatedMetric(k) {
  return /^(cpc|cpm|ctr_pct|cac_krw|cpa_krw|roas|platform_roas|aov|session_cvr_pct|user_cvr_pct|atc_rate_pct|cart_abandonment_pct|revenue_per_user|revenue_per_acquired_user|click_to_purchase_pct|ltv_cac_ratio|ltv_estimate_krw)$/.test(k);
}

// 통합 집계 함수: 지표 특성에 맞는 계산 방식 자동 선택
function aggregateMetric(rows, k) {
  const isPct = /pct|rate|ratio/i.test(k);
  if (isCalculatedMetric(k)) {
    // CPC/CPM 등: 원시 데이터로 가중평균 재계산
    return weightedMetric(rows, k);
  } else if (isPct) {
    // 그 외 % 지표: 단순 평균
    return avgBy(rows, k);
  } else {
  ...
  function draw() {
    ...
    if (dateKey && selectedMetrics.length && currentRange) {
      ...
      selectedMetrics.slice(0, 3).forEach(k => {
        const isPct = /pct|rate|ratio/i.test(k);
        const cur = aggregateMetric(primaryRows, k);
        const prev = aggregateMetric(compareRows, k);
